...
class Building {
  constructor(board, type, x, y, dontPlace, disabled, image) {
    this.board = board;
    this.type = type;
    this.image = image;
    this.x = x;
  ...
  get BoundingBox() {
    return [
      [this.x, this.y],
      [this.x + this.image.width, this.y + this.image.height]
    ];
  }
...
Building.prototype.moveHighlight = function moveHighlight(noFill) {
  if (this.highlight) {
    var highlightX =
      this.sprite.attr("x") -
      (this.data.highlight.width / 2 - this.data.width / 2);
    var highlightY =
      this.sprite.attr("y") -
      (this.data.highlight.height / 2 - this.data.height / 2);
